...
pub(crate) async fn connect_proxy<T>(
    mut tls: T,
    config: &Config,
) -> Result<(Framed<MaybeTlsStream<Socket, T::Stream>, PostgresCodec>, SocketConfig), Error>
where
    T: MakeTlsConnect<Socket>,
{
    ...
    for i in indices {
        ...
        match connect_host(addr, hostname, port, &mut tls, config).await {
            Ok((stream, config)) => return Ok((stream, config)),
            Err(e) => error = Some(e),
        }
    }
    ...
    T: MakeTlsConnect<Socket>,
{
    ...
    let stream = connect_proxy_raw(socket, tls, has_hostname, config).await?;
